...
def branch_sankey(
    ...
    # 🎨 TEMA (dark) — tudo como no original
    DARK_BG = "#0b0f19"
    FONT_CLR = "#e5e7eb"
    TEXT_CLR = "#f9fafb"
    LINK_GREY = "rgba(255,255,255,0.28)"
    ...
    # 🔵 Azul-celeste p/ DESTAQUE (substitui o BLUE do original)
    HILITE_HEX = "#4FC3F7"
    HILITE_LINK = "rgba(79,195,247,0.95)"    # quase opaco para sobressair
    BLUE_LEFT = "rgba(59,130,246,0.50)"      # mantido para as ligações à esquerda (upstream)

    import numpy as np, plotly.express as px

    PALETTE = px.colors.qualitative.Dark24

    # --- calibração para manter links finos (como no original) ---
    CALIBRATE_THIN = True
    DUMMY_A = "\u200b"; DUMMY_B = "\u200c"
    if CALIBRATE_THIN and DUMMY_A not in nodes:
        nodes = nodes + [DUMMY_A, DUMMY_B]
    ...
    # --- mapeamento de ramos a partir do root (como no original) ---
    from collections import defaultdict as _dd, deque as _deq
    children_map = _dd(list)
    for a, b in edges:
        if level.get(a, 0) >= 0 and level.get(b, 0) > 0:
    ...
    # tons neutros por ramo (inalterado)
    BRANCH_TONES = {
        "Alternative rock": "rgba(255,255,255,0.34)",
        "Hard rock":        "rgba(255,255,255,0.34)",
        "Punk rock":        "rgba(255,255,255,0.34)",
    ...
    # === caminho root→focus (robusto a hífens/espacos) ===
    path_edges = set(_path_root_to_focus(root, focus, edges, level))
    path_nodes = {root, focus} | {a for a, _ in path_edges} | {b for _, b in path_edges}

    # pinta NÓS do caminho a azul-celeste (como no original, mas com a nova cor)
    for i, n in enumerate(nodes):
        if n in path_nodes:
    ...
    for a, b in edges:
        ...
        if on_path:
            lcol.append(HILITE_LINK)  # destaque azul-celeste
        elif is_left_edge:
            lcol.append(BLUE_LEFT)    # mantido do original (upstream)
        else:
            if level.get(a, 0) == 0 and level.get(b, 0) == 1:
                lcol.append(TONE_ROOT_DIRECT)
            elif level.get(a, 0) >= 0 and level.get(b, 0) > 0:
                # mantém a tua lógica por ramo (usa 'canonical_name' existente)
                fh = canonical_name(firsthop.get(b) or firsthop.get(a) or "")
                lcol.append(BRANCH_TONES.get(fh, LINK_GREY))
            else:
                lcol.append("rgba(0,0,0,0)" if branch_only else LINK_GREY)
