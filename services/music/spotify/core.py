from services.common.paths import MUSIC_DATA, CINEMA_DATA
...
def load_genres_csv() -> list[str]:
    if os.path.exists("spotify_genres.csv"):
        df = pd.read_csv(MUSIC_DATA / "spotify_genres.csv", sep=';')
        col = None
        for c in df.columns:
            if c.lower() == 'genre':
                col = c
    ...
    if os.path.exists(MUSIC_DATA / "generos.csv"):
        df = pd.read_csv(MUSIC_DATA / "generos.csv", sep=';')
        key = None
        for c in df.columns:
